...
export const CreatorDetailModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="w-full max-w-xl bg-white dark:bg-[#111218] border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {creator.pinnedResources.map(res => (
                <div key={res.id} className="p-4 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-850/40 flex items-center justify-between text-xs">
                  <div>
                  ...
                  <button className="px-3 py-1.5 rounded-lg bg-indigo-600 text-white font-semibold text-[11px] hover:bg-indigo-500 transition-colors cursor-pointer">
                    Access
                  </button>
                </div>
              ))}
          ...
          {creator.playlists.length > 0 && (
            <div className="space-y-3">
              <div className="text-xs font-bold text-zinc-900 dark:text-zinc-100 uppercase tracking-wider">
                Playlists and Curations
              </div>
              <div className="grid grid-cols-2 gap-3">
                {creator.playlists.map(pl => (
                  <div key={pl.id} className="p-3.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-850/40 text-xs">
        ...
        <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between bg-zinc-50/50 dark:bg-zinc-900/50">
          ...
          <button
            ...
            className={`px-5 py-2 rounded-xl text-xs font-semibold cursor-pointer transition-colors ${
              creator.isFollowing ? 'bg-zinc-200 dark:bg-zinc-800 text-zinc-800 dark:text-zinc-200 hover:bg-zinc-300 dark:hover:bg-zinc-700' : 'bg-indigo-600 hover:bg-indigo-500 text-white'
            }`}
          >
