    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (line.trim().startsWith('```')) {
        if (!inCodeBlock) {
          ...
          result.push(
            <div key={`code-block-${currIdx}`} className="my-3 rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-900 text-zinc-100 text-xs shadow-md">
              <div className="px-4 py-2 bg-zinc-950/80 border-b border-zinc-800 flex items-center justify-between text-[11px] text-zinc-400 ">
                ...
                <button
                  ...
                  className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-300 transition-colors text-[10px]"
                >
                  {copiedIndex === currIdx ? (
                    <>
              ...
              <div className="p-4 overflow-x-auto  leading-relaxed text-[11.5px]">
                <pre>{fullCode}</pre>
              </div>
            </div>
          );
