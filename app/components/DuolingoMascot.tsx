...
export const DuolingoMascot: React.FC<DuolingoMascotProps> = ({
  ...
  const currentSvg =
    mascotMap[interactiveEmotion] || mascotMap[emotion] || "/mascot_idle.svg";
  const defaultSpeech =
    speechText ||
    sprint.mascotNarration ||
    `**${user.streak}-day streak**. 20 minutes today keeps your progress active.`;

  return (
    <div
      className={`flex flex-col sm:flex-row items-center sm:items-start gap-4 sm:gap-5 ${className}`}
    >
      {/* Mascot Character */}
      <div
      ...
      >
        <div
          className={`relative ${sizeClasses[size]} transition-transform duration-200 ${
            isBouncing
              ? "animate-bounce"
      ...
      <div className="flex-1 w-full relative">
        <div className="relative p-4 sm:p-5 rounded-2xl bg-white dark:bg-[#111218] border border-zinc-200 dark:border-zinc-800 shadow-xs space-y-2.5">
          {/* Speech Bubble Arrow Tail for Desktop */}
              ...
              return (
                <div className="pt-2 border-t border-zinc-100 dark:border-zinc-800/60 flex flex-wrap items-center gap-2">
                  <button
                    ...
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] font-medium shadow-xs transition-colors cursor-pointer"
                  >
                  ...
                  <button
                    onClick={() => setMascotOpen(true)}
                    className="px-3 py-1.5 rounded-xl border border-zinc-200 dark:border-zinc-700 hover:border-zinc-300 dark:hover:border-zinc-600 bg-zinc-50 dark:bg-zinc-800/60 text-zinc-700 dark:text-zinc-300 text-[11px] font-medium transition-colors cursor-pointer"
                  >
